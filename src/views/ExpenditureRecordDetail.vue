...
const fetchCategories = async () => {
  ...
  onMounted(async () => {
    ...
    try {
      const response = await axios.get(`/api/expenditure/detail/${recordId}`); // Fetch the record details
...
if (message) {
  ...
  const deleteRecord = async () => {
    try {
      await axios.delete(`/api/expenditure/detail/${record.value._id}`); // Delete the record from the server
