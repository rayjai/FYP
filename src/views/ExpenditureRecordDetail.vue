...
onMounted(async () => {
  try {
    ...
    const response = await axios.get(`/api/expenditure/detail/${route.params.id}`);
...
const updateRecord = async () => {
  try {
    ...
    await axios.put(`/api/expenditure/detail/${record.value._id}`, editedRecord.value);
...
const deleteRecord = async () => {
  try {
    await axios.delete(`/api/expenditure/detail/${record.value._id}`);
