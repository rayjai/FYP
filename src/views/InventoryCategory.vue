    ...
    <div>
        ...
        <div class="action-buttons" style="display: flex; justify-content: flex-end; margin-bottom: 10px;">
            <button class="btn btn-expenditure" @click="openFormModal">
                <i class='bx bx-dollar'></i> Add Inventory Category
            </button>
        </div>
        <ag-grid-vue
...
const handleSubmit = async () => {
    ...
    try {
        const response = await axios.post(`/api/inventory_category`, {
        ...
        if (response.status === 200) {
            localStorage.setItem('toastrMessage', 'Inventory Category added successfully!');
